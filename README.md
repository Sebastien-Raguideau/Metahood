...
```
### Databases (needed):
We rely on Checkm hmm for MAG quality assesment:
Please download: `https://data.ace.uq.edu.au/public/CheckM_databases/checkm_data_2015_01_16.tar.gz`

#### Databases (Optional)
##### Kegg orthologies with kofamscan
Metahood can optionally launch [kofamscan](https://github.com/takaram/kofam_scan) on your assembly and tabulate KO orthologies. To do so please download the 2 folloing files:

    https://www.genome.jp/ftp/db/kofam/ko_list.gz
    https://www.genome.jp/ftp/db/kofam/profiles.tar.gz


##### Contig taxonomic annotation with CAT
Contig taxonomic annotation is possible using the tool [CAT](https://github.com/dutilh/CAT#installation)
To use this tool, please download and decompress the related database:
```
wget tbb.bio.uu.nl/bastiaan/CAT_prepare/CAT_prepare_20210107.tar.gz
tar -xvzf CAT_prepare_20210107.tar.gz
```
##### User defined diamond database
Custom annotation using diamond is possible using metahood. To do so, please activate the Metahood conda environment before using diamond to build the database;

    diamond makedb --in <fasta.file> -db <db name>
...
- **annotation**: this folder contain all annotation files and content will depend on config file:
	...
	- contigs_SCG.fna: fasta file of all single copy core genes founds in the assembly.
- **assembly**: output directory of megahit
- **binning**: by default metahood run both concot and metabat2 and produce a consensus of both. Thus three folder are produced. In each folder, can be found:
	- clustering_\<binner\>.csv: csv file, first column is the contig, second the bin number/name
	- \<binner\>_MAG_list.txt: list of bins deemed to be MAGs.
	- bins: folder with one fasta file per bin.
...
- **profile**: 
	- coverage_contigs.tsv: table of coverage of all contigs in all samples
	- coverage_orfs.tsv: table of coverage of all orfs in all samples
	- coverage_contigs_C10K.tsv: table of coverage of all splits contigs in all samples for concoct.
	- cov_\<annotation\>.tsv: coverage of annotations id annotated. For instance kegg orthology annotation can be obtained if proper database path are supplied in the config file, then a file names cov_KEGG.tsv will be generated and will correspond to summation of coverage of orfs annotated under the same Kegg orthology. The same file is generated for all annotation done defined under diamond in the config file.
	- mag_\<binner\>_coverage.tsv : per sample coverage of mags generated by \<binner\>.
	- mag_\<binner\>_percent_mapped.tsv: per sample percentage of reads mapped onto mags from \<binner\>.
	- mapping_percent.tsv: percent reads mapped per samples.
	- Normalisation.tsv: number of nucleotide per sample and mean Single copy Core Gene coverage per sample. 
