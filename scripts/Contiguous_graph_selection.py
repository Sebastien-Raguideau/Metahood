...
def Crawl_through_subgraph_node(node_dir,Set_node,Dico_contig_degree,depth) :
    if depth<MAX_DEPTH:
        ...
        if List_New_node :
            for New_node in List_New_node :
                ...
                if (New_node,Direction) not in Set_node : 
                    ...
                    Crawl_through_subgraph_node((New_node,Direction),Set_node,Dico_contig_degree,depth+1)
...
def contiguous_nodes_extraction(args) :
    def Crawl_through_subgraph_node(node_dir,Set_node,Dico_contig_degree,depth) :
        if depth<MAX_DEPTH:
            (node,Direction)=node_dir
            List_New_node=Dico_contig_degree[node][Direction]
            if List_New_node :
                for New_node in List_New_node :
                    ...
                    if (New_node,Direction) not in Set_node : 
                        Set_node.add((New_node,Direction))
                        Crawl_through_subgraph_node((New_node,Direction),Set_node,Dico_contig_degree,depth+1)
    List_nodes_ini,Dico_contig_degree = args
    Set_contigs2=set()
    for index,contig in enumerate(List_nodes_ini) :
        for Direction in [0,1] :
            local_set={(contig,Direction)}
            Crawl_through_subgraph_node((contig,Direction),local_set,Dico_contig_degree,0)
            if len(local_set)>1 :
...
def graph_selection(List_nodes,Gfa_file,ouptut_file) :
    ...
    # default dict can't be pickled because defaultdic was not created in a top level function
    # https://stackoverflow.com/questions/16439301/cant-pickle-defaultdict
    Dico_contig_degree_cp = {key:val for key,val in Dico_contig_degree.items()}
    missing_contig = {contig for contig in Dico_contig_line if contig not in Dico_contig_degree_cp}
    Dico_contig_degree_cp.update({contig:[set(),set()] for contig in missing_contig})
    ...
    pool =Pool(THREADS)
    args = [(List_nodes[index::THREADS],Dico_contig_degree_cp) for index in range(THREADS)]
    res = pool.map(contiguous_nodes_extraction,args)
    del pool # whatever you do, only this seems to really close the pool. pool.close/

    res=[]
    for index,arg in enumerate(args):
        print("arg nb %s"%index)
        %time res.append(contiguous_nodes_extraction(arg))

    Set_nodes_selected = set().union(*res)
