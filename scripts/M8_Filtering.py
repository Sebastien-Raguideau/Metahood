...
def main(m8_file, Database_file, min_Bitscore, max_Evalue, min_Pid, min_ref_pid, min_coverage, min_Query_coverage):
    ...
    print("\t".join(["Query", "Subject", "Bitscore", "PID",
                     "Subject_Pid", "Coverage", "Query_coverage"]+Header[2:]))
    for List_annotation in generate_filtered_Annotation(generate_all_annotation(m8_file)):
        for annotation in List_annotation:
            ...
            # must divide by 100. to get a real percentage
            PID = float(Splitline[9])/100
            ...
            if (Bitscore >= min_Bitscore) & (max_Evalue >= Evalue) & (PID >= min_Pid) & (Subject_Pid >= min_ref_pid) & (Coverage >= min_coverage) & (Query_coverage >= min_Query_coverage):
                if Database_file:
                    try:
                        ...
                        print("\t".join([Query, Subject, str(Bitscore), "{:.3f}".format(PID), "{:.3f}".format(
                            Subject_Pid), "{:.3f}".format(Coverage), "{:.3f}".format(Query_coverage)]), file=sys.stderr)
                        continue
                else:
                    Newname = Subject
                    Additional_annotation = []
                Result = [Query, Newname, str(Bitscore), "{:.3f}".format(PID), "{:.3f}".format(
                    Subject_Pid), "{:.3f}".format(Coverage), "{:.3f}".format(Query_coverage)]+Additional_annotation
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "m8_file", help=" m8 output, columns should be 'qseqid sseqid qstart qend qlen sstart send slen length pident evalue bitscore'")
    ...
    parser.add_argument(
        "-B", help="minimum value cutoff for Bitscore", default=0)
    parser.add_argument("-E", help="maximum value for Evalue", default=1e-5)
    parser.add_argument(
        "-P", help="cutoff for PID, between 0 and 1", default=0)
    parser.add_argument(
        "-R", help="Subject pid : minimum (Subject_Coverage x percentage_identity), between 0 and 1", default=0)
    parser.add_argument(
        "-C", help="Subject Coverage cutoff : minimum percentage of the subject the Alignment does cover, between 0 and 1 ", default=0)
    parser.add_argument(
        "-Q", help="Querry Coverage cutoff : minimum percentage of the Querry the Alignment does cover, between 0 and 1 ", default=0)
