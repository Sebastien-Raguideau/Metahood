...
def main(bam_file,out_stats,out_hists,maxlen):
    ...
    with open(out_stats,"w") as handle_stat,open("%s"%out_hists,"w") as handle_hists2:
        # handle_hists.write("contig\tposition\tcoverage\n")
        handle_hists2.write("contig\tdepth\tnb_pos\n")
        ...
        for contig,length in contigs_10K.items():
            ...
            if med!=0:
                ...
                handle_stat.write("%s\t%s\t%s\n"%(contig,med,"NA"))
            # stats
            actual_hist = Counter(depthArray)
            covs = sorted(actual_hist.keys())
            handle_hists2.writelines("%s\t%s\t%s\n"%(contig,c,actual_hist[c]) for c in covs)
