...
def collate_in_memory(cov_files, get_sample_name, output):
...
def collate_dictionary(cov_files, get_sample_name, output):
    sorted_files = sorted(cov_files)
    samples_names = sorted([ get_sample_name(path) for path in sorted_files])
...
if __name__ == "__main__":
    ...
    suffix = args.s
    if suffix:
        get_sample_name = lambda x:basename(x).replace(suffix,"")
    else:
    ...
    if len(cov_files) < 1020:
        ...
        collate_iterator(cov_files, get_sample_name, output)
    else:
        collate_in_memory(cov_files, get_sample_name, output)
